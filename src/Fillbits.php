<?php
require('FillbitsCurlRequest.php');

/**
 * Fillbits.com PHP API Wrapper
 *
 * @link https://www.filbits.com/ Official Fillbits.com API Documentation.
 * @copyright (c) 2019, Fillbits.com
 *
 * Many API commands require a currency or currencies values to be passed. These are always in the
 * format of a currency ticker code. See https://www.filbits.com/supported-tokens for these ticker codes,
    ...
    public function __construct($public_key, $username, $password, $format = "json")
    {
        ...
        try {
            if (empty($this->public_key) || empty($this->username) || empty($this->password)) {
                throw new Exception("Your public key, username or password are not set!");
            }
        } catch (Exception $e) {
